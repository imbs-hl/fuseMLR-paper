...
create_pred_fuseMLR_best <- function(data_name_list, output_folder, suffix,
  ...
  for(sz in 1:length(data_name_list)) {
    ...
    for (i in 1:rep) {
      ...
      result <- create_pred_fuseMLR(entities = get(data_name)[[i]],
                                    ...
                                    meta_lrn_param_list = list(), # Parameter for MetaLearner
...
create_pred_fuseMLR_best(data_name_list = data_name_list, output_folder = result_best_dir,
                         suffix = "best", rep = 100, seed_values = seed_values,
                         ranger_param = ranger_param_prob)
