...
create_pred_fuseMLR_mia <- function(data_name_list, output_folder, suffix,
  ...
  for(sz in 1:length(data_name_list)) {
    ...
    for (i in 1:rep) {
      ...
      result <- create_pred_fuseMLR(entities = get(data_name)[[i]],
                                    ...
                                    meta_lrn_id = "mia",
                                    lrn_fct = "ranger",
                                    meta_lrn_param_list = list(num.trees = 1000,
                                                               probability = TRUE,
                                                               na.action = "na.learn"), 
...
create_pred_fuseMLR_mia(data_name_list = data_name_list, output_folder = result_mia_dir,
                        suffix = "mia", rep = 100, seed_values = seed_values,
                        ranger_param = ranger_param_prob)
