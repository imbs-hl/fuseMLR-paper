...
# function
create_pred_fuseMLR_cobra <- function(data_name_list, output_folder, suffix,
  ...
  for(sz in 1:length(data_name_list)) {
    ...
    for (i in 1:rep) {
      ...
      result <- create_pred_fuseMLR(entities = get(data_name)[[i]],
                                    ...
                                    meta_lrn_id = "cobra_epsilon",
                                    lrn_fct = "cobra",
                                    meta_lrn_param_list = list(tune = "epsilon", k_folds = 10), # Parameter for MetaLearner
...
create_pred_fuseMLR_cobra(data_name_list = data_name_list, output_folder = result_cobra_dir,
                          suffix = "cobra_epsilon", rep = 100, seed_values = seed_values,
                          ranger_param = ranger_param_prob)
